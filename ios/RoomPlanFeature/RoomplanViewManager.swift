...
class RoomplanViewManager: RCTViewManager {
    ...
    // settle) and translates the tag into the live RoomplanView instance,
    ...
    // see RealityKitViewManager.swift's withView for the same pattern
    // applied to the other native view in this app.

    private func withView(_ reactTag: NSNumber, _ action: @escaping (RoomplanView) -> Void) {
    ...
    @objc func startScanning(_ reactTag: NSNumber) {
        withView(reactTag) { $0.startScanning() }
    }

    @objc func stopScanning(_ reactTag: NSNumber) {
        withView(reactTag) { $0.stopScanning() }
    }

    @objc func resetScanning(_ reactTag: NSNumber) {
        withView(reactTag) { $0.resetScanning() }
    }

    @objc func abortScanning(_ reactTag: NSNumber) {
        withView(reactTag) { $0.abortScanning() }
    }
}
...
class RoomplanView: UIView, RoomCaptureSessionDelegate {
    ...
    func stopScanning() {
        DispatchQueue.main.async { [weak self] in
            ...
            // Just stop the session; the view stays so the frozen preview stays
